...
% Data generated by dataForQ_bySession_all.m
% Stan model run and output generated with QLearning_all_bySession_2022.py (qLearning_stay_side_session_v2.stan) 
% QDiff is right - left 
% QChosenDiff is chosen - unchosen

% Generate pathname for fit data
filebasename = fullfile(whereAreWe('figurecode'),'processed_data','qModel_all');
% Load animal list and filelist
load(fullfile(filebasename,'all_data_bySession.mat'),'aids','flist_all');

% Load stan paramters for each session
%stanParams = readtable(fullfile(filebasename,'summary_int_pc.csv'));
% Load subject index of sessions
load(fullfile(filebasename, 'all_toStan.mat'),'subj_idx','NT_all','NS_all','c','r');

% Load mouse level parameter estimates 
load(fullfile(filebasename,'alphas_mice.mat'));
load(fullfile(filebasename,'betas_mice.mat'));
load(fullfile(filebasename,'stays_mice.mat'));
load(fullfile(filebasename,'sides_mice.mat'));
alphas_mice = alphas;
betas_mice = betas;
stays_mice = stays;
sides_mice = sides;
...
load(fullfile(filebasename,'alphas.mat'));
load(fullfile(filebasename,'betas.mat'));
load(fullfile(filebasename,'stays.mat'));
load(fullfile(filebasename,'sides.mat'));


for na = 1:numel(aids) 
    fprintf('running %s \n',aids{na});
    filename   = fullfile(whereAreWe('figurecode'),'processed_data','Operant', aids{na}); % where to save the qlearning results
    mkdir(filename); 
    probCounter = 0; 
    ...
    save(fullfile(filename, 'qLearn_session_all.mat'),'qLearn');
    qLearn = qLearn_mouse;
    save(fullfile(filename, 'qLearn_animal_all.mat'),'qLearn'); 
